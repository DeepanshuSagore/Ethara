...
1. **Auth:** ✅ Demo Mode (role switcher) + Normal Mode (Clerk login with role-based access).
   _Final scope (Phase 9): shipped **Demo Mode only** — the role switcher is the default landing
   and covers every evaluator flow; Clerk was descoped to keep the live demo friction-free._
...
- [x] Phase 5 — Seed Data ✅ deterministic + idempotent Faker seeder (`python -m app.seed.run`: FK-ordered wipe + bulk insert in one transaction, fixed seed/base date so reruns are byte-identical); exact §5b targets hit — 11 named ACTIVE projects, 5 floors × 2 zones × 80 bays × 7 = 5,600 seats, 5,000 employees (Amit Sharma / amit@ethara.ai as #1, unique emails, mock's dept→role mapping), 4,940 ACTIVE allocations clustered per project home zone, 510 AVAILABLE / 100 RESERVED / 50 MAINTENANCE / 4,940 OCCUPIED, 50 PENDING / 10 EXITED / 25 ON_LEAVE; `app.seed.verify` prints a summary + 21 hard checks (targets + invariants) and exits non-zero on failure; verified independently via sqlite3 CLI, double-run hash-identical, pytest still 10/10
- [x] Phase 6 — REST APIs ✅ five routers at the exact spec paths (root-mounted, verbatim in Swagger — locked by a pytest on openapi.json), thin over `app/services/`: allocation service enforcing rules 1–4 with friendly 409s + IntegrityError races translated, release/deactivate sharing one atomic RELEASED+AVAILABLE transition, rule-5 suggestion ranking (team zone → same floor → alternate) mirroring the mock's `suggestSeatsFor`, dashboard metrics computed live per request (rule 8), `POST /ai/query` deterministic keyword engine answering seat/availability/project/utilization questions from the DB (brief's amit@ethara.ai example verified); convenience extras `GET /projects/{id}`, `GET /seats/{id}`, `GET /seats/suggestions`, `limit`/`offset`; proper 201/404/409/422 everywhere; pytest 47/47 (37 new endpoint tests: every rule happy + rejection); all paths + rejections curl-verified against a live server on the seeded DB
- [x] Phase 7 — Integration ✅ mock store fully replaced by the real API: TanStack Query + typed fetch client in `src/lib/api/` (one module per resource, `NEXT_PUBLIC_API_URL` env, ApiError surfacing FastAPI `detail`), hooks mirroring the mock's surface (dashboard summary/project/floor, employees with **server-side** search/filters + limit/offset paging, projects, per-floor seat map, rule-5 suggestions, who-sits-where seat index over the new read-only `GET /allocations` convenience endpoint, allocate/release/add-joiner mutations invalidating the cache so dashboard/seat-map/lists refetch live), assistant on `POST /ai/query`, detail-route `generateMetadata` fetching the API with graceful fallback, `src/lib/mock` deleted (and all ×20 display scaling with it), Phase 3 loading/empty/error states driven by real fetch status; verified in headless Chrome 25/25 (4,990/5,600/88% dashboard, amit search → A1-1, allocate→4,941 without reload, release restores, assistant answers the brief's query); lint/tsc/build green, backend pytest 47/47 untouched, DB reseeded after verification
- [x] Phase 8 — AI Assistant ✅ Groq NL layer (`app/services/ai_nl.py`) in front of the Phase 6 keyword engine per §2: one httpx POST (llama-3.3-70b-versatile, JSON mode, temp 0, 4s timeout) parses NL → structured intent (8 intents: the four Phase 6 ones + `floor_most_available`, `project_on_floor`, `off_topic`, `unknown`), executors run against the DB reusing the Phase 6 answer composers so every fact comes from real rows; deterministic engine is the guaranteed fallback on missing key / HTTP error / timeout / rate limit / bad JSON / unknown intent / low confidence — endpoint contract unchanged, never 500s, works offline; guardrails: 500-char cap before any network call, scoped refusal for off-topic/prompt-injection (strict intent whitelist + confidence floor rejects derailed parses), model-echoed text sanitized + capped, key backend-only and never logged; pytest 66/66 fully offline (autouse blank-key fixture; 19 new tests mock httpx with real Response objects covering parse-success per intent, garbage/429/timeout/connect-error → fallback, refusal, guardrails); verified live via curl (brief's example + natural paraphrases with the real key, then `GROQ_API_KEY=""` override → all fallback answers) and headless Chrome 6/6 on /assistant; frontend copy-only change (badge + welcome), lint/tsc/build green
- [x] Phase 9 — Deploy + Final Docs ✅ everything driven through the Render/Vercel APIs (zero dashboard clicks): Render free Postgres 17 in Oregon (external access allow-listed to the dev machine's /32 only) migrated + seeded from local — `app.seed.verify` 21/21 against prod; Render web service `ethara-api` (root `backend`, PYTHON_VERSION pinned 3.14.3 matching the Phase 4 pins, DATABASE_URL on the internal network with the `postgresql+psycopg://` scheme, Groq key set via API); Vercel project `ethara` (root `frontend`, `NEXT_PUBLIC_API_URL` baked at build, GitHub repo linked → auto-deploys on push) → **https://ethara-snowy.vercel.app** over **https://ethara-api-edmu.onrender.com**; CORS updated to the Vercel aliases + redeployed; live verification: curl suite (4,990/5,600/88% summary, brief's amit@ethara.ai query via Groq, off-topic refusal, all 12 spec paths in openapi.json) + a real Groq-down drill (key **deleted** — Render's API 400s on empty values — deterministic fallback proven with a Groq-only intent, key restored) + headless-Chrome pass on production; screenshots/ captured from the live app; DEPLOYMENT.md (steps, env, free-tier gotchas: 30-day Postgres expiry 2026-08-12, ~50s cold starts), README live URLs, AI_PROMPTS §9, DEBUGGING_NOTES §Phase 9 (4 deploy-time gotchas)
